...
def test_prediction_acc(model, tok, hparams, prompts, targets, device, locality=False, vanilla_generation=False):
    if vanilla_generation:
        ...
        for prompt, target_new in zip(prompts, targets):
            ...
            if isinstance(prompt, dict):
                #print(prompt)
                prompt_tok = tok.apply_chat_template(
                    Chat([prompt]),
                ...
                #print("testtesttest")
                #print(prompt_tok["input_ids"])
            
            #print("origin target new tokens")
            #print(len(target_new_tokens))
            start = time.time()
            ...
            #print(gen_token)
            #print(len(prompt_tok['input_ids']))
            #print(target_new)
            #print(target_new_tokens)
            #print(len(gen_token))
            gen_txt = [tok.decode(x, skip_special_tokens=True) for x in gen_token.detach().cpu().numpy().tolist()[0][-len(target_new_tokens):]]
            gen_txt_1 = ''.join(gen_txt).strip()
            #print(target_new_tokens)
            answer_txt = [tok.decode(x, skip_special_tokens=True) for x in target_new_tokens]
    ...
    with torch.no_grad():
        ...
        if isinstance(answers[0], list):
            ...
            return [np.mean(np.equal(answers, labels)),txt]
