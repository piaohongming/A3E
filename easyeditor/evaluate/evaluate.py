...
def compute_edit_quality(
    ...
    if rephrase_prompts is None:
        rephrase_prompts = record["rephrase"] if 'rephrase' in record.keys() else None
...
def compute_rewrite_or_rephrase_quality(
    ...
    if eval_metric == 'ppl':
        ...
        if 't5' in model_name.lower():
            ...
            acc, exec_time = test_prediction_acc(model, tok, hparams, prompt, target_new, device, vanilla_generation=True)
        #f1 = F1(model,tok,hparams,prompt,target_new,device, vanilla_generation=True)
        ret = {
            f"{key}_acc": acc,
            f"{key}_exec_time": exec_time,
            # f"{key}_PPL": ppl,
            #f"{key}_F1":f1,     
        }        
    else:
...
def compute_locality_quality(
    ...
    if 't5' in model_name.lower():
        ...
        #print("begin local")
        loc_tokens, exec_time = test_prediction_acc(model, tok, hparams, prompt, locality_ground_truth, device, locality=True, vanilla_generation=True)
        #print("end local")
    ...
    ret = {
        ...
        f"{locality_key}_exec_time": exec_time,
    }
    return ret
...
def compute_multimodal_edit_quality(model, batch, exach_match=False, tok=None):
    ...
    answers = slice_list(answers,[len(mask)-int(mask.sum().float().item())-1, len(mask)],left=False)
    txt = [tok.decode(x, skip_special_tokens=True) for x in answers]
    txt = [
    ...
    return [acc.numpy().tolist(), txt], pred_ids.numpy(), logits_
...
def compute_multimodal_edit_results(
    ...
    ret['rewrite_acc'], _, _ = compute_multimodal_edit_quality(model, edit_inner, tok=tok)
    ...
    if "rephrase_prompt" in record.keys():
        ...
        ret['rephrase_acc'], _, _ = compute_multimodal_edit_quality(model, edit_outer, tok=tok)
    ...
    if "image_rephrase" in record.keys():
        ...
        ret['image_rephrase_acc'], _, _ = compute_multimodal_edit_quality(model, edit_image_outer, tok=tok)
    ...
    if 'locality_prompt' in record.keys():
        ...
        _, _, ret['locality_output'] = compute_multimodal_edit_quality(model, locality, tok=tok)
    ...
    if 'multimodal_locality_prompt' in record.keys():
        ...
        _, _, ret['multimodal_locality_output'] = compute_multimodal_edit_quality(model, m_locality, tok=tok)
    # Form a list of lists of prefixes to test.
