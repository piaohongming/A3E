...
class BaseEditor:
    ...
    def __init__(self,
        ...
        if type(self.model_name) is str:
            ...
            if 't5' in self.model_name.lower():
                ...
                #self.model = AutoModelForCausalLM.from_pretrained(self.model_name, torch_dtype=torch_dtype, device_map=device_map, max_memory={0: "0GiB", 1: "31GiB", 2: "0GiB", 3: "0GiB", 4: "31GiB", 5: "0GiB", 6: "0GiB", 7: "31GiB"})
                self.model = AutoModelForCausalLM.from_pretrained(self.model_name, torch_dtype=torch_dtype, device_map=device_map, max_memory={0: "0GiB", 1: "0GiB", 2: "31GiB", 3: "0GiB", 4: "0GiB", 5: "31GiB", 6: "31GiB", 7: "0GiB"})
    ...
    def edit(self,
        ...
        test_set = []
        all_metrics_mutual = copy.deepcopy(all_metrics)
        if self.alg_name == "ROME" and "spatial" not in kwargs['pre_file'] and "zsre" not in kwargs['pre_file'] and "counterfact" not in kwargs['pre_file'] and "recent" not in kwargs['pre_file']:
            pass
        elif self.alg_name != "ROME" and ("zsre" in kwargs['pre_file'] or "counterfact" in kwargs['pre_file'] or "recent" in kwargs['pre_file']):
            pass
        ...
        for i, request in enumerate(requests):
            ...
            if "zsre" in kwargs['pre_file'] or "counterfact" in kwargs['pre_file'] or "recent" in kwargs['pre_file']:
                if self.alg_name == "ROME":
                    edited_model, weights_copy, origin = self.apply_algo(
                        self.model,
                        self.origin_model,
                        self.tok,
                        [request],
                        ...
                        edit_type=self.hparams.edit_type,
                        memory=self.memory
                    )
                else:
                    edited_model, weights_copy = self.apply_algo(
            ...
            elif ("sd_sc" in kwargs['pre_file'] or "dc" in kwargs['pre_file'] or "port" in kwargs['pre_file'] or "multi" in kwargs['pre_file'] or "sequential" in kwargs['pre_file']) and "mutual" not in kwargs['pre_file']:
                if requests[i]['edit_type'] == "complete" or requests[i]['edit_type'] == "reasoning":
                #if i == len(requests) - 1 or requests[i]['origin_edit_id'] != requests[i+1]['origin_edit_id'] or requests[i]['edit_type'] == "reasoning":
                    #print("reset!!!!!!!!!!!!")
                    start = time()
                    all_metrics[i].update({
                        #'origin_edit_id': requests[i]['origin_edit_id'],
                        'edit_type': requests[i]['edit_type'],
                        'case_id': i,
                        "requested_rewrite": request,
                        "time": 0,
                    ...
                    if self.alg_name == "ROME" or self.origin_model is None:
                        pass
                    else:
                        self.model.to("cpu")
                        edited_model.to("cpu")
                        self.model = copy.deepcopy(self.origin_model)
                    '''
            ...
            elif ("ss" in kwargs['pre_file'] or "ds" in kwargs['pre_file'] or "dd" in kwargs['pre_file'] or "dc" in kwargs['pre_file']) and "mutual" in kwargs['pre_file']:

                test_set.append({"case_id": i, "request": request})
                if request["edit_type"] != "complete":
                    if self.alg_name == 'IKE':
                        assert 'train_ds' in kwargs.keys(), print('IKE need train_ds(For getting In-Context prompt)')
                        edited_model, weights_copy, icl_examples = self.model, {}, self.apply_algo(
                            self.model,
                ...
                if i == len(requests) - 1 or requests[i]['origin_edit_id'] != requests[i+1]['origin_edit_id']:
                    for saved_request_dict in test_set:
                        saved_request_id = saved_request_dict["case_id"]
                        saved_request = saved_request_dict ["request"]
                        start = time()
                        all_metrics_mutual[saved_request_id].update({
                            'origin_edit_id': saved_request['origin_edit_id'],
                            'edit_type': saved_request['edit_type'],
                            'case_id': saved_request_id,
                            "requested_rewrite": saved_request,
                            "time": 0,
                            "post": compute_edit_quality(edited_model, self.model_name, self.hparams, self.tok, saved_request, self.hparams.device, test_generation=test_generation),
                        })
                        if "metric_kwargs" in kwargs:
                            all_metrics_mutual[saved_request_id].update(compute_sent_metric(self.model, edited_model, self.model_name, self.hparams, self.tok, metric_kwargs=kwargs["metric_kwargs"][saved_request_id], device=self.hparams.device))
                        
                        '''
                        if 'locality' in all_metrics_mutual[saved_request_id]['post'].keys():
                            for locality_key in saved_request['locality'].keys():
                                assert len(all_metrics_mutual[saved_request_id]['post']['locality'][f'{locality_key}_output']) == \
                                    len(all_metrics_mutual[saved_request_id]['pre']['locality'][f'{locality_key}_output'])
                                locality_result = []
                                for ans,label in zip(all_metrics_mutual[saved_request_id]['post']['locality'][f'{locality_key}_output'],all_metrics_mutual[saved_request_id]['pre']['locality'][f'{locality_key}_output']):
                                    locality_result.append(np.mean(np.equal(ans, label)))
                                all_metrics_mutual[saved_request_id]['post']['locality'][f'{locality_key}_acc'] = locality_result
                                all_metrics_mutual[saved_request_id]['post']['locality'].pop(f'{locality_key}_output')
                            all_metrics_mutual[saved_request_id]['pre'].pop('locality')
                        '''
                        LOG.info(f"Evaluation took {time() - start}")

                        if verbose:
                            LOG.info(
                                f"{saved_request_id} mutual editing: {saved_request['prompt']} -> {saved_request['target_new']}  \n {all_metrics_mutual[saved_request_id]}"
                            )
                    test_set = []
                    if self.alg_name == "ROME":
                        pass
        ...
        if "zsre" in kwargs['pre_file'] or "counterfact" in kwargs['pre_file'] or "recent" in kwargs['pre_file']:
            for i, request in enumerate(requests):
                ...
                all_metrics[i].update({
                    ...
                    "requested_rewrite": request,
                    "post": compute_edit_quality(edited_model, self.model_name, self.hparams, self.tok, request, self.hparams.device, test_generation=test_generation),
                    #"post2": compute_edit_quality(self.origin_model, self.model_name, self.hparams, self.tok, request, self.hparams.device, test_generation=test_generation),
                })
                if "metric_kwargs" in kwargs:
                    all_metrics[i].update(compute_sent_metric(self.model, edited_model, self.model_name, self.hparams, self.tok,metric_kwargs=kwargs["metric_kwargs"][i], device=self.hparams.device))
                ...
                if 'locality' in all_metrics[i]['post'].keys():
                    for locality_key in request['locality'].keys():
                        locality_result = []
                        for ans, label in zip(all_metrics[i]['post']['locality'][f'{locality_key}_output'], all_metrics[i]['pre']['locality'][f'{locality_key}_output']):
                            gen_txt = [self.tok.decode(x, skip_special_tokens=True) for x in ans]
                            gen_txt_1 = ''.join(gen_txt).strip()
                            answer_txt = [self.tok.decode(x, skip_special_tokens=True) for x in label]
                            answer_txt_1 = ''.join(answer_txt).strip()
                            scorer = rouge_scorer.RougeScorer(['rouge1', 'rouge2', 'rougeL'], use_stemmer=False, tokenizer=self.tok)
                            
                            locality_result.append(scorer.score(gen_txt_1, answer_txt_1))
                            #locality_result.append(np.mean(np.equal(ans, label)))
                        all_metrics[i]['post']['locality'][f'{locality_key}_acc'] = locality_result
                        all_metrics[i]['post']['locality'].pop(f'{locality_key}_output')
                    all_metrics[i]['pre'].pop('locality')
        ...
        return all_metrics, all_metrics_mutual, None, weights_copy
    ...
    def simple_edit(self,
        ...
        edited_model, weights_copy, label_tok = self.apply_algo(
            self.model,
            self.tok,
            [request],
            self.hparams,
            ...
            success_answers=kwargs['success_answers']
        )
        return edited_model, label_tok
    ...
    def _prepare_requests(self,
        ...
        if 'edit_type' in kwargs and kwargs['edit_type'] is not None:
            if isinstance(kwargs['edit_type'], str):
                kwargs['edit_type'] = [kwargs['edit_type'],]
            else:
                #pass
                assert len(kwargs['edit_type']) == len(prompts)
            '''
            for prompt_, subject_ in zip(prompts, kwargs['subject']):
                assert subject_ in prompt_, print(f'Subject:{subject_} do not exist in prompt: {prompt_}')
            '''
            for i, request in enumerate(requests):
                request.update(
                    {
                        'edit_type': kwargs['edit_type'][i]
                    }
                )

        if 'origin_edit_id' in kwargs and kwargs['origin_edit_id'] is not None:
            if isinstance(kwargs['origin_edit_id'], str):
                kwargs['origin_edit_id'] = [kwargs['origin_edit_id'],]
            else:
                assert len(kwargs['origin_edit_id']) == len(prompts)
            '''
            for prompt_, subject_ in zip(prompts, kwargs['subject']):
                assert subject_ in prompt_, print(f'Subject:{subject_} do not exist in prompt: {prompt_}')
            '''
            for i, request in enumerate(requests):
                request.update(
                    {
                        'origin_edit_id': kwargs['origin_edit_id'][i]
                    }
                )
