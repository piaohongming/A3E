    ...
    def edit(self,
        ...
        for i, request in enumerate(requests):
            ...
            if self.alg_name == 'IKE':
                ...
                if 'locality_output' in all_metrics[i]['post'].keys():
                    assert len(all_metrics[i]['post']['locality_output']) == \
                            len(all_metrics[i]['pre']['locality_output'])
                    base_logits = all_metrics[i]['pre']['locality_output'].to(torch.float32)
                    post_logits = all_metrics[i]['post']['locality_output'].to(torch.float32)
                    ...
                    all_metrics[i]['post']['locality_acc'] = (sum(post_base_logits_softmax_top_k.view(-1) == base_logits_softmax_top_k.view(-1))/post_base_logits_softmax_top_k.view(-1).shape[0]).numpy().tolist()
                    all_metrics[i]['post'].pop('locality_output')
                    all_metrics[i]['pre'].pop('locality_output')
                    
                    
                if 'multimodal_locality_output' in all_metrics[i]['post'].keys():
                    assert len(all_metrics[i]['post']['multimodal_locality_output']) == \
                            len(all_metrics[i]['pre']['multimodal_locality_output'])
                    base_image_logits = all_metrics[i]['pre']['multimodal_locality_output'].to(torch.float32)
                    post_image_logits = all_metrics[i]['post']['multimodal_locality_output'].to(torch.float32)
                    ...
                    all_metrics[i]['post']['multimodal_locality_acc'] = (sum(post_image_base_logits_softmax_top_k.view(-1) == base_image_logits_softmax_top_k.view(-1))/post_image_base_logits_softmax_top_k.view(-1).shape[0]).numpy().tolist()
                    all_metrics[i]['post'].pop('multimodal_locality_output')
                    all_metrics[i]['pre'].pop('multimodal_locality_output')
                LOG.info(f"Evaluation took {time() - start}")
                all_metrics[i]['requested_rewrite'].pop('image')
                all_metrics[i]['requested_rewrite'].pop('image_rephrase')
                all_metrics[i]['requested_rewrite'].pop('multimodal_locality_image')
                if verbose:
                    LOG.info(
                ...
                all_metrics[i].update({
                    ...
                    "requested_rewrite": request,
                    "time": exec_time,
                    "post": compute_multimodal_edit_results(edited_model, self.model_name, self.hparams, self.tok, request, self.hparams.device, test_generation=test_generation),
                })
