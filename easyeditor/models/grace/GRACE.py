...
class GRACE(torch.nn.Module):
    def __init__(self, config, model, device):
        ...
        if type(original_layer) is not GRACEAdapter:
            setattr(edit_module, layer_name, GRACEAdapter(config, original_layer, transpose=transpose))
    ...
    def edit(self, config, tokens, edit_id):
        global RE_LOSS
        #print("CWMCWMCWM:{}".format(tokens["labels"]))
        if "instruct" in config.model_name.lower():
        ...
        for i in range(config.n_iter):
            ...
            if i == 0:
                origin_distribution = outputs.logits
            ...
            dis_loss = 1000 * self._ntd_loss(outputs.logits, origin_distribution, tokens["labels"][0][-1]).to(outputs.loss.device)
            #max_prob_loss = (outputs.logits[tokens["labels"][0][-1]]-torch.max(origin_distribution))**2
            loss = outputs.loss# + dis_loss# + RE_LOSS.to(outputs.loss.device)
            #loss = outputs.loss + self._ntd_loss(outputs.logits, origin_distribution, tokens["labels"][0][-1]) + RE_LOSS
            print("loss:{} ce_loss:{} dis_loss:{} re_loss:{}".format(loss, outputs.loss, dis_loss, RE_LOSS))
    ...
    def forward(self, *args):
        ...
        if (not self.training) & ('keys' not in self.__dict__):
            ...
            if not self.training and self.key_id == -1:
                ...
                self.query = args[0][:, token_to_edit, :]
        ...
        if self.training:
            ...
            smallest_dist, self.chosen_key = dists.topk(dists.shape[0], dim=0, largest=False, sorted=False)
            smallest_dist, self.chosen_key = smallest_dist[-1].unsqueeze(0), self.chosen_key[-1].unsqueeze(0)
            #print(self.chosen_key)
            smallest_dist, self.chosen_key = smallest_dist.squeeze(1), self.chosen_key.squeeze(1)
            print(self.chosen_key)
        else:
            smallest_dist, self.chosen_key = dists.topk(dists.shape[0], dim=0, largest=False, sorted=False)
            #smallest_dist, self.chosen_key = dists.topk(1, dim=0, largest=False, sorted=False)
            #smallest_dist, self.chosen_key = smallest_dist[-1].unsqueeze(0), self.chosen_key[-1].unsqueeze(0)
            #print(self.chosen_key)
            smallest_dist, self.chosen_key = smallest_dist.squeeze(1), self.chosen_key.squeeze(1)
        #print(smallest_dist.shape)
        #print(self.chosen_key.shape)
        ...
        #print(query)
        #print(smallest_dist, eps)
        #print(token_to_edit)
        #print(layer_out.shape)
        if not self.training:
            layer_out = layer_out.repeat(dists.shape[0], 1, 1)
        if self.replacement == "replace_all":
            ...
            #layer_out[:, token_to_edit] = chosen_value
            #print(self.training)            
            if not self.training and layer_out.shape[1] != 1:
                ...
                #print(smallest_dist <= eps)
                previous_hidden = layer_out[:, token_to_edit].detach().clone()
                layer_out[:, token_to_edit] = torch.where((smallest_dist <= eps), chosen_value, layer_out[:, token_to_edit])
                RE_LOSS = (torch.mean(previous_hidden) - torch.mean(layer_out[:, token_to_edit]))**2 + 1 * (torch.var(previous_hidden) - torch.var(layer_out[:, token_to_edit]))**2
                #RE_LOSS = (torch.mean(previous_hidden) - torch.mean(layer_out[:, token_to_edit]))**2 + 1 * (torch.max(previous_hidden) - torch.max(layer_out[:, token_to_edit]))**2
        ...
        if not self.training:
            if layer_out.shape[1] != 1:
                print("layer out shape: {}".format(layer_out.shape))
                layer_out[0][-1] = 0.07*layer_out[0][-1] #本来是0.05,0.07(Persian China)
                layer_out[1][-1] = 0.07*layer_out[1][-1] #现在是0.07,0.07(America, Japan)
                print("max 0: {}".format(torch.max(layer_out[0][-1])))
                print("mean 0: {}".format(torch.mean(layer_out[0][-1])))
                print("max 1: {}".format(torch.max(layer_out[1][-1])))
                print("mean 1: {}".format(torch.mean(layer_out[1][-1])))
            else:
                print("after max 0: {}".format(torch.max(layer_out[0][-1])))
                print("after mean 0: {}".format(torch.mean(layer_out[0][-1])))
                print("after max 1: {}".format(torch.max(layer_out[1][-1])))
                print("after mean 1: {}".format(torch.mean(layer_out[1][-1])))
            
            layer_out = torch.sum(layer_out, dim=0, keepdim=True)
            #print(layer_out.shape)
        return layer_out
