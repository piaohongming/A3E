...
from .a3e_hparams import A3EHyperParams
from .a3e_multimodal_hparams import A3EMultimodalHyperParams
from .util import get_tokenizer
from .a3e import LORA
from ...util import nethook


def apply_a3e_to_model(
        model: AutoModelForCausalLM,
        tok: AutoTokenizer,
        requests: List[Dict],
        hparams: A3EHyperParams,
        copy=False,
...
def apply_a3e_to_multimodal_model(
    model: AutoModelForCausalLM, 
    tok: AutoTokenizer,
    requests: List[Dict],
    hparams: A3EHyperParams,
    ...
    **kwargs: Any,
    ...
    device = torch.device(f'cuda:{hparams.device}')
    if not isinstance(model, LORA):
        editor = LORA(model, hparams,None)
    else:
        editor = model
