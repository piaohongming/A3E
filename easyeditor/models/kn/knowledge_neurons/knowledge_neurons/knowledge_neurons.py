...
class KnowledgeNeurons:
    ...
    def _prepare_inputs(self, prompt, target=None, encoded_input=None):
        if encoded_input is None:
            ...
            if isinstance(prompt, dict):
                addition = prompt['addition']
                prompt.pop("addition")
                encoded_input_ori = self.tokenizer.apply_chat_template(Chat([prompt]), return_tensors="pt", add_generation_prompt=True, return_dict=True).to(self.device)
                encoded_input_add = self.tokenizer(addition, return_tensors="pt").to(self.device)
                #print(encoded_input_ori)
                #print(encoded_input_add)
                encoded_input = {}
                encoded_input["input_ids"] = torch.cat((encoded_input_ori["input_ids"], encoded_input_add["input_ids"]), dim=1)
                encoded_input['attention_mask'] = torch.cat((encoded_input_ori['attention_mask'], encoded_input_add['attention_mask']), dim=1)
            else:
                encoded_input = self.tokenizer(prompt, return_tensors="pt").to(self.device)
    ...
    def get_scores(
        ...
        batch_size: int = 10,
        steps: int = 20,
        attribution_method: str = "max_activations",
        pbar: bool = True,
    ):
        """
        ...
        print(prompt)
        if isinstance(prompt, dict):
            encoded_input = self.tokenizer.apply_chat_template(Chat([prompt]), return_tensors="pt", add_generation_prompt=True, return_dict=True).to(self.device)
        else:
            encoded_input = self.tokenizer(prompt, return_tensors="pt").to(self.device)
    ...
    def get_scores_for_layer(
        ...
        encoded_input, mask_idx, target_label = self._prepare_inputs(
            copy.deepcopy(prompt), ground_truth, encoded_input
        )
        ...
        if attribution_method == "integrated_grads":
            ...
            for i in range(n_sampling_steps):
                if i > 0 and (self.model_type == "qwen" or self.model_type == "gpt" or self.model_type == 'llama'):
                    # retokenize new inputs
                    encoded_input, mask_idx, target_label = self._prepare_inputs(
                        copy.deepcopy(prompt), ground_truth
                    )
                (
                    baseline_outputs,
                ...
                if n_sampling_steps > 1:
                    if isinstance(prompt, dict):
                        prompt["addition"] = prompt["addition"] + next_token_str
                        #print(prompt)
                    else:
