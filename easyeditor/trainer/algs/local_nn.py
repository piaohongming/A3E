...
class LRLinear(nn.Module):
    ...
    def forward(self, x, mode=None):
        ...
        pre_act = (self.u.to(x.device) @ (self.v.to(x.device) @ x.T)).T
        if self.bias is not None:
            pre_act += self.bias.to(x.device)

        if mode is not None:
            if not isinstance(mode, torch.Tensor):
                mode = torch.tensor(mode).to(x.device)
            scale, shift = self.mode_scale(mode.to(self.mode_scale.weight.device)), self.mode_shift(mode.to(self.mode_shift.weight.device))
            pre_act = pre_act * scale.to(x.device) + shift.to(x.device)
