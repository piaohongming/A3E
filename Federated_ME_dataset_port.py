...
for i in range(len(original_dataset) - 1):
    ...
    if "Reasoning" in original_dataset[i]["portability"].keys():
        ...
        for reasoning in reasoning_list:
            ...
            messages_revise = copy.deepcopy(messages_revise_template)
            messages_revise[1]["content"] = messages_revise[1]["content"].format(hop2_labels)
            outputs = pipeline(
                messages_revise,
                max_new_tokens=512,
                eos_token_id=terminators,
            ...
            '''
            try:
                revise_json = json.loads(outputs[0]["generated_text"][-1]["content"])
            except:
                continue
            '''
            #print(revise_json)
            revise_concept = outputs[0]["generated_text"][-1]["content"]
            #print(hop2_labels)
            #print(revise_concept)
